...
int slider = 255;
int led = 0;
int servoAngle = 0; // по умолчанию 180 градусов
...
<div class="parent">
  ...
  <div class="div3">
    ...
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0.5rem;">
      <div style="font-size:1.5rem;margin-bottom:0.5rem;">SERVO: <span id="servo-txt">0</span>°</div>
      <input type="range" id="servo-val" min="0" max="180" value="0" style="width:150px;height:20px;">
    </div>
  </div>
</div>
...
const servoTxt = document.getElementById("servo-txt");
const servoVal = document.getElementById("servo-val");

servoVal.addEventListener('input', () => {
  const angle = parseInt(servoVal.value);
  servoTxt.textContent = angle;
  send({servoAngle: angle});
});
...
String getSensorReadings(){
  ...
  readings["servo"] = String(servoAngle);  // ← добавляем текущий угол серво
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    else if (myObject.hasOwnProperty("servoAngle")) {
        int angle = (int)myObject["servoAngle"];
